...
function initFourierImage() {
    ...
    $('#FTransform').addEventListener('click', function() {
		...
		index=0;
		currImageData2 = d.createImageData(dims[1],dims[0]);
		for (var k = dims[0]-1; k >=0 ; k--) {
			for (var l = 0; l < dims[1]; l++) {
				var idxInPixels2 = 4*(dims[0]*l + k);
				for (var c = 0; c < 3; c++) { 
					currImageData2.data[index+c] = currImageData.data[idxInPixels2+c];
				}
				index+=4;
				currImageData2.data[index+3] = 255;
			}
		}
		
		for (var k = dims[1]/2; k < dims[1] ; k++) {
			for (var l = 0; l < dims[0]/2; l++) {
				var idxInPixels2 = 4*(dims[0]*k + l);
				var idxInPixels3 = 4*(dims[0]*(k+1)-2- l);
				for (var c = 0; c < 3; c++) { 
					var temp = currImageData2.data[idxInPixels2+c];
					currImageData2.data[idxInPixels2+c]=currImageData2.data[idxInPixels3+c];
					currImageData2.data[idxInPixels3+c]=temp;					
				}
			}
		}		
		ctxs[1].putImageData(currImageData2, 0, 0);
